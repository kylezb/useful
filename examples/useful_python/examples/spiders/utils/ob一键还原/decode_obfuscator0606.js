...
function getObfuscatorCode(path,funcNames)
{
	let obfuscatorCode = "";
	...
	if (!isElementsLiteral(path.get('init')))
	{
		return "";
	}
	...
	obfuscatorCode = parentPath.toString();	
	...
	for (const referPath of referencePaths)
	{
		...
		if (Paths.includes(AncestorPath))
		{
			continue;
		}
		obfuscatorCode += ";\n" + AncestorPath.toString();
		Paths.push(AncestorPath);		
	}
	if (Paths.length != 2)
	{
		obfuscatorCode = "";
...
const CallExpressToLiteral = 
{
	VariableDeclarator(path) 
	{
		let {scope} = path;
    let funcNames = [];
    let code = getObfuscatorCode(path, funcNames);
   
    if (code.length == 0) return;
    
    code += getReDefineFunction(scope,funcNames);
    
    let funcAst = parser.parse(code);
	  code = generator(funcAst,opts = {"compact":true}).code;
	  
    eval(code);
    
    let binding =  scope.getBinding(funcNames[0]);
    let end = binding.path.node.end;
    let canRemoved = true;
