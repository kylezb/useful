...
````
定义: 
    function FunctionName(Iparametersl){public private linternal lexternal} [pure constant view payable] [modifiers] [returns(return types)]
...
# truffle 使用
# 报错:Unbox failed!  Error: getaddrinfo ENOENT raw.githubusercontent.com
```
替换host文件为:
